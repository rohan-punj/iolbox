...
// off Linux. The tap and the sudo-driven ip/iptables setup only work inside the
// runtime; the pure logic (subnet allocation, DHCP codec, feature gating) is
// tested on any OS.
var ErrUnsupportedPlatform = errors.New("extnet: tap endpoints are only supported on linux")
