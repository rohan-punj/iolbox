#!/usr/bin/env bash
# 50-verify-native.sh — prove that the native-arm64 guest is ready and
# persistent.
#
# This is the native-arm64 counterpart of 50-verify.sh. It is IDENTICAL to
# it except for two truthfulness-critical differences: the loader canary
# checked here is the native aarch64 loader (not the Rosetta amd64 one), and
# the correlated hello check requires arch aarch64 (not x86_64) — a native
# guest whose supervisor still reports arch x86_64 has not proven anything
# useful, even if every other check here passes.
#
# In order, this step:
#   1. polls systemd and ss until the supervisor is active and the GUI socket
#      is genuinely bound;
#   2. polls GET / until its HTTP status is below 500;
#   3. asserts the native loader canary, image registry, licence, and
#      supervisor;
...
# lib.sh unconditionally sets IOLBOX_LOADER to the amd64 loader path; this
# profile's hard gate is the native aarch64 loader.
IOLBOX_LOADER="${IOLBOX_NATIVE_LOADER:-/lib/ld-linux-aarch64.so.1}"
...
verify_capability_hello() {
    ...
    if ! printf '%s\n' '{"id":"macos-verify-hello","op":"hello","args":{"client":"50-verify-native"}}' >&3; then
        exec 3>&-
        exec 3<&-
        return 1
    fi
...
main() {
    ...
    [ "$(id -u)" -eq 0 ] || die "$IOLBOX_EXIT_PREFLIGHT" \
        "50-verify-native.sh must run as root"
