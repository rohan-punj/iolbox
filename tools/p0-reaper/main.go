...
func main() {
	...
	setprivPath := flag.String("setpriv", "", "optional setpriv executable")
	flag.Parse()
	if *target == "" || *result == "" {
		fmt.Fprintln(os.Stderr, "usage: p0-reaper --target PATH --result PATH [--setpriv PATH]")
		os.Exit(2)
	}
	if err := runReaper(*target, *result, *setprivPath); err != nil {
