...
func main() {
	...
	// Interactive sizing: a double-clicked exe gets asked the two questions
	// that matter (Enter-Enter accepts the 4/4096 defaults); an explicitly
	// passed --smp/--mem is respected without nagging, and a piped/scripted
	// stdin never prompts at all — see prompt.go.
	opts.smp, opts.memMB = promptResources(
		stdinIsInteractive(),
		explicitFlags(func(f func(string)) { flag.Visit(func(fl *flag.Flag) { f(fl.Name) }) }),
		opts.smp, opts.memMB)
