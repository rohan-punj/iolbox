#!/usr/bin/env bash
# install.sh — installs iolab onto ANY systemd x86-64 glibc Linux server
...
#   sudo ./install.sh [--bind local|all] [--prefix /opt/iolab]
#
...
#   4. write /etc/iolab/bind.env (local or all) + the bind drop-in
#   5. enable + start iolab-firstboot-iourc.service and iolab-supervisor.service
...
if ! command -v systemctl >/dev/null 2>&1 || [ ! -d /run/systemd/system ]; then
    ...
    echo "  iolab's autostart units (iolab-supervisor.service,"  >&2
    echo "  iolab-firstboot-iourc.service) require systemd. Refusing to"  >&2
...
install.sh: WARNING - this host's hostname ('$CUR_HOSTNAME') looks like it
  may be cloud-init-managed or auto-generated. iolab's IOL license (iourc)
...
install -m 0755 -o root -g root "$SCRIPT_DIR/opt-iolab/firstboot-iourc.sh" "$PREFIX/firstboot-iourc.sh"
install -m 0755 -o root -g root "$SCRIPT_DIR/opt-iolab/prestart-clean.sh" "$PREFIX/prestart-clean.sh"

echo "install.sh: installing systemd units"
install -m 0644 "$SCRIPT_DIR/systemd/iolab-supervisor.service" \
    /etc/systemd/system/iolab-supervisor.service
install -m 0644 "$SCRIPT_DIR/systemd/iolab-firstboot-iourc.service" \
    /etc/systemd/system/iolab-firstboot-iourc.service

install -d -m 0755 /etc/systemd/system/iolab-supervisor.service.d
install -m 0644 "$SCRIPT_DIR/systemd/10-bind.conf" \
    /etc/systemd/system/iolab-supervisor.service.d/10-bind.conf

echo "install.sh: writing /etc/iolab/bind.env (--bind $BIND_MODE)"
install -d -m 0755 /etc/iolab
if [ "$BIND_MODE" = "all" ]; then
    install -m 0644 "$SCRIPT_DIR/systemd/bind.env.all" /etc/iolab/bind.env
else
    install -m 0644 "$SCRIPT_DIR/systemd/bind.env.local" /etc/iolab/bind.env
fi

echo "install.sh: installing sysctl drop-in"
install -d -m 0755 /etc/sysctl.d
install -m 0644 "$SCRIPT_DIR/etc/99-iolab.conf" /etc/sysctl.d/99-iolab.conf
...
systemctl enable iolab-firstboot-iourc.service iolab-supervisor.service
...
systemctl start iolab-firstboot-iourc.service
if [ -f "$PREFIX/iourc" ]; then
    ...
    echo "    journalctl -u iolab-firstboot-iourc.service" >&2
fi

systemctl start iolab-supervisor.service
...
sleep 2
if ! systemctl is-active --quiet iolab-supervisor.service; then
    echo "install.sh: WARNING - iolab-supervisor.service is not active." >&2
    echo "  Check: journalctl -u iolab-supervisor -e" >&2
fi
...
============================================================
 ...
 Bind mode: $BIND_MODE
   (change later: edit /etc/iolab/bind.env, then
    systemctl restart iolab-supervisor.service)
 ...
 only — never expose this host's iolab ports to the public
 internet (no port-forwarding a cloud instance's :4001, no
 opening it in a public-facing security group).

 Logs:   journalctl -u iolab-supervisor -f
 Status: systemctl status iolab-supervisor iolab-firstboot-iourc
============================================================
EOF
