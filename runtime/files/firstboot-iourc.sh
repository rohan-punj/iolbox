...
# fallback init path (files/iolbox-init.sh) on minimal images that lack
# systemd, so keep it portable — no bashisms.
set -eu

IOLBOX_DIR="/opt/iolbox"
IOURC_FILE="$IOLBOX_DIR/iourc"
MARKER="$IOLBOX_DIR/.iourc-generated"
...
if [ ! -x "$SUPERVISOR" ]; then
    ...
    # packaging bug that iolbox-supervisor.service will also fail loudly on.
    exit 0
fi
...
# ---------------------------------------------------------------------------
if "$SUPERVISOR" -gen-iourc > "$IOURC_FILE.tmp" 2>"$IOLBOX_DIR/.iourc-gen.log"; then
    ...
    rm -f "$IOURC_FILE.tmp"
    echo "firstboot-iourc: supervisor -gen-iourc failed, see $IOLBOX_DIR/.iourc-gen.log" >&2
