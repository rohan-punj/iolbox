...
func main() {
	...
	app := NewApp(store)
	if err := app.receiver.Start(store.Snapshot().ListenPort); err != nil {
		log.Printf("syslog: UDP listener failed: %v", err)
	}
	if !hasLabIface() {
		log.Printf("syslog: eth1 is not wired yet; the service will be reachable after the node gets a lab address")
	}
...
func fail(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "syslog-gui: "+format+"\n", args...)
	os.Exit(1)
}
